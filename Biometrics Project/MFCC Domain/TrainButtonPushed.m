function TrainButtonPushed(app, event)
    ...
    fid = fopen('mfcc_feature.dat', 'w');
    
    % Display the word after silence detection
    figure;
    subplot(2, 2, 1);
    plot(x);
    xlabel("Samples"); ylabel("Normalized Amplitude");
    title("Speech Signal After Silence Detection");
    
    % Display the word after windowing using a Hamming window
    subplot(2, 2, 2);
    plot(x1);
    xlabel("Samples"); ylabel("Amplitude");
    title("Speech Signal After Hamming Windowing");
    
    % Display the Fourier transform after preprocessing
    subplot(2, 2, 3);
    frequencies = linspace(0, fs, length(mx));
    plot(frequencies, abs(mx));
    xlabel("Frequency (Hz)"); ylabel("Magnitude");
    title("Fourier Transform After Preprocessing");
    
    % Display the spectrum after Mel-warping
    subplot(2, 2, 4);
    melFrequencies = mel(f);
    plot(melFrequencies, nx);
    xlabel("Mel Frequency"); ylabel("Amplitude");
    title("Spectrum After Mel-Warping");
